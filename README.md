...
| 2. [프로젝트 개요](#프로젝트-개요) | 12. [세그먼트와 캠페인](#세그먼트와-캠페인) |
...
| 7. [문제 정의](#문제-정의) | 17. [실행 방법](#실행-방법) |
| 8. [시스템 구조](#시스템-구조) | 18. [한계와 향후 개선](#한계와-향후-개선) |
| 9. [데이터와 예측 시점](#데이터와-예측-시점) | 19. [관련 문서](#문서) |
| 10. [데이터 처리 흐름](#데이터-처리-흐름) | |
...
KKBOX의 회원·결제·음악 이용 로그를 활용해 구독 만료 후 이탈 가능성을 예측하고, 예측 결과를 고객 가치와 구독 생명주기에 결합해 `Retention`, `긴급 갱신`, `Win-back` 캠페인으로 연결했습니다. 전처리와 모델링부터 MySQL 서빙 DB, FastAPI, 관리자 캠페인 콘솔, 고객 알림·혜택 화면까지 하나의 흐름으로 구현한 데이터 프로젝트입니다.
...
| 문성호 | 처음에는 모델의 성능을 높이는 것에 집중했지만, 프로젝트를 진행하면서 높은 성능보다 올바른 데이터와 예측 시점을 설계하는 것이 더 중요하다는 것을 배웠습니다. 특히 지나치게 높은 성능을 의심해 데이터 누수를 발견하고, 관측 시점을 다시 설정해 모델을 재구성하는 과정이 가장 기억에 남았습니다. 또한 단순히 모델을 학습하는 데서 끝나는 것이 아니라, 고객의 결제 및 이용 행동을 피처로 만들고 이탈 위험도를 실제 리텐션 캠페인과 서비스로 연결하면서 데이터 분석 결과가 실제 비즈니스에서 어떻게 활용될 수 있는지도 경험할 수 있었습니다.|
