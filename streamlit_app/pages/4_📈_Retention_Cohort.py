...
retention = pd.read_csv(DATA_DIR / "retention_cohort.csv").set_index("cohort_month")
retention.columns = [int(c) for c in retention.columns]

fig = go.Figure(go.Heatmap(
    z=retention.values, x=[f"M{m + 1}" for m in retention.columns], y=retention.index,
...
jun2015 = json.loads((DATA_DIR / "jun2015_investigation.json").read_text(encoding="utf-8"))
